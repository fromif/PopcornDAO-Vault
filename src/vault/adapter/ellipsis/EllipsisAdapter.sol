...
import { IEllipsis, IStaking, IAddressProvider } from "./IEllipsis.sol";
...
contract EllipsisAdapter is AdapterBase, WithRewards {
    ...
    */
    ...
    function _totalAssets() internal view override returns (uint256) {
        ...
        uint256 lpBalance = IStaking(ellipsisLPStaking).userInfo(lpToken, address(this)).depositAmount;
        ...
        override
    {
        if (address(ellipsisLPStaking) != address(0)) {
            ...
            IStaking(ellipsisLPStaking).deposit(lpToken, lpTokenBalance, false);
        }
    }
        ...
        override
    {
        ...
        IStaking(ellipsisLPStaking).withdraw(lpToken, _lpShare, false);
        uint256 n_coins = IAddressProvider(addressProvider).get_n_coins(ellipsisPool);
    ...
    function claim() public override onlyStrategy returns (bool success) {
        ...
        _rewardTokens[0] = IStaking(ellipsisLPStaking).rewardToken();
        try 
        IStaking(ellipsisLPStaking).claim(address(this), _rewardTokens) {
            success = true;
        } catch {}
    }
